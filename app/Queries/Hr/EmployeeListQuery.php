    ...
    public function handle(EmployeeFilterDto $dto): Builder
    {
        return Employee::with([
            ...
            ->when(! empty($dto->entities), function ($query) use ($dto) {
                return $query->whereHas('entities', function ($query) use ($dto) {
                    $query->whereIn('_entities.id', $dto->entities);
                });
            })
            ->when(! empty($dto->colleges), function ($query) use ($dto) {
                return $query->whereHas('colleges', function ($query) use ($dto) {
                    $query->whereIn('_colleges.id', $dto->colleges);
                });
            })
